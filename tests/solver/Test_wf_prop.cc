...
WilsonFermionR Dw(Umu,Grid,RBGrid,mass);
    ...
    */  
    ...
    int mu;
    int nu;
    LatticeComplex C1(&Grid);
    LatticeComplex C2(&Grid);
    std::vector <TComplex> C1t;
    std::vector <TComplex> C2t;
    //Bilinears (non-singlet)   <O1(y)O2(x)>= - < g5 * G1 * S_2(x,y) * G2 * g5 * adj(S_1(x,y)) >
    //    C1 = (-1.)*trace(Gamma(g[5])*(Gamma(g[5])*Prop*Gamma(g[5])*(Gamma(g[5])*adj(Prop))));
    mu=5;
    nu=5;


    C1 = trace(Gamma(g[mu])*Prop*Gamma(g[nu])*Gamma(g[5])*(adj(Prop)*Gamma(g[5]))); // -1 ?    
    sliceSum(C1,C1t,3);
    //    C2 = (-1.)*trace(Gamma(g[nu])*Prop*Gamma(g[mu])*(Gamma(g[5])*adj(Prop)));
    mu=5;
    nu=4;
    C2 = trace(Gamma(g[mu])*Prop*Gamma(g[nu])*(adj(Prop)*Gamma(g[5])));    // -1 ?
    sliceSum(C2,C2t,3);
    std::cout << "t     <PP>     <PA0>" << std::endl;
    for (int t=0;t<T;t++){
      std::cout << " " << t << "     " << TensorRemove(C1t[t]) << "     " << TensorRemove(C2t[t]) << std::endl;
    } 
